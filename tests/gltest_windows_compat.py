...
The upstream direct loader removes an open fd-0 temporary file before Windows
releases its duplicated handle. Its time warp also fails to refresh
``gl.message_raw['datetime']``. Keep this test-only shim until upstream fixes
land; it is never part of the deployed contract.
"""
...
def install_windows_direct_compatibility() -> None:
    """Install idempotent Windows-only direct runner patches."""
    ...
    if getattr(loader, "_questionzero_windows_compat", False):
        return
    ...
    def inject_message_to_fd0(vm):
        ...
        try:
            ...
            os.dup2(fd, 0)
            vm._questionzero_stdin_path = path
        finally:
            os.close(fd)
    ...
    def cleanup_after_deactivate(vm):
        path = getattr(vm, "_questionzero_stdin_path", None)
        try:
            original_cleanup(vm)
        finally:
            if path:
                try:
                    ...
                    pass
                vm._questionzero_stdin_path = None
    ...
    loader._questionzero_windows_compat = True


def install_glsim_direct_compatibility() -> None:
    """Keep GLSim's cached storage class pointed at the real contract instance."""
    install_windows_direct_compatibility()

    import glsim.engine as engine_module

    if getattr(engine_module, "_questionzero_direct_compat", False):
        return
    ...
    engine_module._questionzero_direct_compat = True
